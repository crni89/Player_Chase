...
export default class AuthMiddleware {
    public static getVerifier(... allowedRoles: ("player"|"agent")[]): (req: Request, res: Response, next: NextFunction) => void {
        return (req: Request, res: Response, next: NextFunction) => {
            this.verifyAuthToken(req, res, next, allowedRoles);
        }
    }

    private static verifyAuthToken(req: Request, res: Response, next: NextFunction, allowedRoles: ("player"|"agent")[]) {
    ...
    public static validateTokenAs(tokenString: string, role: "player" | "agent", type: "auth" | "refresh"): ITokenData {
        if (tokenString === "") {
            throw {
                status: 400,
